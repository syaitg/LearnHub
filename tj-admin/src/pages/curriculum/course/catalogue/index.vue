...
// 获取课程目录接口返回的错误提示
const getCatalogueErrorMessage = (error, defaultMessage) => {
  return error?.response?.data?.msg || error?.message || defaultMessage;
};
// 将后端业务提示转换为用户容易理解的操作说明
const formatCatalogueErrorMessage = (message) => {
  if (message === "不能出现空章") {
    return "课程目录中存在没有小节的章节，请为每个章节添加至少一个小节，或者删除空章节后再保存！";
  }
  return message || "课程目录保存失败，请检查目录内容后重试！";
};
// 1552958577757843500
// 提交
const handleSubmit = async (str) => {
  ...
  if (itemData.value.length === 0) {
    ...
    for (var i = 0; i < itemData.value.length; i++) {
      const chapter = itemData.value[i];
      if (!chapter.name || chapter.name.trim() === "") {
        isName.value = false;
        ElMessage({
          showClose: true,
          message: `第${i + 1}章的名称为空，请输入章名称后再保存！`,
          type: "warning",
          duration: 5000,
        });
        return false;
...
// 前端先拦截空章节，后端仍保留相同校验作为最终保障
      if (!Array.isArray(chapter.sections) || chapter.sections.length === 0) {
        ElMessage({
          showClose: true,
          message: `章节“${chapter.name}”下还没有小节，请添加至少一个小节，或者删除该章节后再保存！`,
          type: "warning",
          duration: 5000,
        });
        return false;
      ...
      for (var j = 0; j < chapter.sections.length; j++) {
        if (!chapter.sections[j].name || chapter.sections[j].name.trim() === "") {
          isKnobbleName.value = false;
          ElMessage({
            showClose: true,
            message: `章节“${chapter.name}”中的第${j + 1}个小节名称为空，请填写后再保存！`,
            type: "warning",
            duration: 5000,
          });
          return false;
      ...
      await baseCatalogueSave(params)
        .then((res) => {
          if (res.code === 200) {
            ...
            ElMessage({
              showClose: true,
              message: formatCatalogueErrorMessage(res?.msg),
              type: "error",
              duration: 5000,
            });
          }
        ...
        .catch((err) => {
          ElMessage({
            showClose: true,
            message: formatCatalogueErrorMessage(
