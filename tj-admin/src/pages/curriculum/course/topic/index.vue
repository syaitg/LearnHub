...
<template>
  <div class="contentBox">
              ...
              <ul>
                <li v-for="(val, i) in item.sections" :key="i">
                  ...
                  <div class="con">
                    ...
                    <div>
                      <div v-if="val.type === 3" class="stage-test-actions">
                        <span class="textDefault ai-entry" @click="handleAiQuestionOpen(item, val)">AI生成题目</span>
                        <span class="textWarning" @click="handleDeleteOpen(val)">删除阶段考试</span>
                      </div>
                    </div>
                  </div>
                </li>
    ...
    <!-- end -->
    <!-- 删除弹层 -->
    <!-- AI 出题工作台：复用 AI 教学工作台的生成、审核、确认和发布能力。 -->
    <el-drawer v-model="aiWorkspaceVisible" title="阶段考试 AI 出题工作台" size="92%" destroy-on-close>
      <AiEducation
        v-if="aiWorkspaceVisible"
        :key="aiWorkspaceKey"
        embedded
        :initial-course-id="aiWorkspaceContext.courseId"
        :initial-scope-type="aiWorkspaceContext.scopeType"
        :initial-scope-id="aiWorkspaceContext.scopeId"
        :initial-target-biz-id="aiWorkspaceContext.targetBizId"
        @published="handleAiPublished"
      />
    </el-drawer>
    <Delete
...
// 复用 AI 教学工作台，不复制生成、审核和发布逻辑
import AiEducation from "@/pages/aiEducation/index.vue";
// ------定义变量------
// 获取父组件值、方法
...
// AI 工作台上下文；打开前先保存当前目录，使刚添加的阶段考试立即成为可用发布目录
const aiWorkspaceVisible = ref(false);
const aiWorkspaceKey = ref(0);
const aiWorkspaceSaving = ref(false);
const aiWorkspaceContext = reactive({
  courseId: courseId,
  scopeType: "CHAPTER",
  scopeId: "",
  targetBizId: "",
...
/**
 * 打开嵌入式 AI 工作台。
 * 阶段考试刚添加时可能还只存在于当前编辑数据中，因此先静默保存目录，
 * 再把当前章节和阶段考试传给原有 AI 工作台，避免用户保存、退出后重新选择课程。
 */
const handleAiQuestionOpen = async (chapter, stageTest) => {
  if (!courseId || !chapter?.id || !stageTest?.id || aiWorkspaceSaving.value) return;
  aiWorkspaceSaving.value = true;
  try {
    const response = await baseCatalogueSave({
      datas: itemData.value,
      id: courseId,
      step: 4,
    });
    if (response?.code !== 200) {
      return ElMessage.error(response?.msg || response?.data?.msg || "保存课程目录失败，暂时无法打开 AI 出题");
    }
    Object.assign(aiWorkspaceContext, {
      courseId: String(courseId),
      scopeType: "CHAPTER",
      scopeId: String(chapter.id),
      targetBizId: String(stageTest.id),
    });
    aiWorkspaceKey.value += 1;
    aiWorkspaceVisible.value = true;
  } finally {
    aiWorkspaceSaving.value = false;
  }
};
/** AI 题目发布成功后刷新课程题目数量，编辑页无需退出即可看到最新结果。 */
const handleAiPublished = async () => {
  await getCatalogue();
  await getSubject();
};
