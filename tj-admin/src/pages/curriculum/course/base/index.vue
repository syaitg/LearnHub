...
let isUploading = ref(false); //记录课程封面是否正在上传
...
const validateUpload = (rule, value, callback) => {
  if (typeof fromData.value.coverUrl === "string" && fromData.value.coverUrl.trim()) {
    callback();
  } else {
...
const handleSubmit = async (str) => {
  ...
  flag.value = Boolean(
    typeof fromData.value.coverUrl === "string" && fromData.value.coverUrl.trim()
  );
...
const getCoverUrl = (val) => {
  ...
  flag.value = Boolean(val);
};
// 记录图片上传状态，避免在上传完成前保存临时预览图
const setUploadStatus = (val) => {
