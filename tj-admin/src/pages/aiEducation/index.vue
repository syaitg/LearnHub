<template>
  <div :class="['ai-page', 'container', { 'embedded-workspace': embedded }]">
    <div v-if="!embedded" class="ai-hero">
      <div>
      ...
      <el-tab-pane label="题目草稿审核" name="drafts">
        <div class="workspace-grid">
          <el-card class="panel-card create-card" shadow="never">
            <template #header>
              <div class="card-title"><span>创建 AI 出题批次</span>
                <el-tag type="primary">P1-A</el-tag>
              </div>
            </template>
            <el-alert title="视频测验也会进入这条草稿确认和发布流程，不会产生第二套题库。" type="info" :closable="false"
                      show-icon/>
            <el-form ref="batchFormRef" :model="batchForm" label-width="92px" class="workspace-form">
              <CourseCataloguePicker ref="batchPickerRef" v-model:course-id="batchForm.courseId"
                                     v-model:scope-type="batchForm.scopeType" v-model:scope-id="batchForm.scopeId"
                                     v-model:target-biz-id="batchForm.targetBizId" :show-media="false"
                                     :course-readonly="embedded"
                                     :scope-readonly="embedded"
                                     :target-readonly="embedded"
                                     :allow-chapter="embedded"/>
              <el-form-item label="来源类型" class="is-required">
                <el-select v-model="batchForm.sourceType">
                  <el-option label="手工主题" value="MANUAL_TOPIC"/>
                  <el-option label="材料文本" value="MATERIAL_TEXT"/>
                </el-select>
              </el-form-item>
              <el-form-item v-if="batchForm.sourceType === 'MANUAL_TOPIC'" label="知识点" class="is-required">
                <el-input v-model="batchForm.knowledgePoints" type="textarea" rows="3" maxlength="5000" show-word-limit
                          placeholder="请输入本小节需要考查的知识点，多个知识点可用换行或逗号分隔。"/>
              </el-form-item>
              <el-form-item v-if="batchForm.sourceType === 'MATERIAL_TEXT'" label="出题材料" class="is-required">
                <el-input v-model="batchForm.materialText" type="textarea" rows="4" maxlength="20000" show-word-limit
                          placeholder="请输入课程材料，提示词和生成内容均使用中文。"/>
              </el-form-item>
              <el-form-item label="题型" class="is-required">
                <el-checkbox-group v-model="batchForm.questionTypes">
                  <el-checkbox v-for="item in questionTypeOptions" :key="item.value" :label="item.value">{{
                      item.label
                    }}
                  </el-checkbox>
                </el-checkbox-group>
              ...
              <div class="form-row">
                <el-form-item label="题目数量" class="is-required">
                  <el-input-number v-model="batchForm.questionCount" :min="1" :max="20"/>
                </el-form-item>
                <el-form-item label="难度" class="is-required">
                  <el-select v-model="batchForm.difficulty">
                    <el-option v-for="item in difficultyOptions" :key="item.value" :label="item.label"
                               :value="item.value"/>
                  </el-select>
                </el-form-item>
                <el-form-item label="每题分值" class="is-required">
                  <el-input-number v-model="batchForm.score" :min="1" :max="100"/>
                </el-form-item>
              </div>
              <el-form-item>
                <el-button type="primary" :loading="batchCreating" @click="createBatch">开始生成草稿</el-button>
          ...
          <el-card class="panel-card" shadow="never">
            <template #header>
              <div class="card-title"><span>出题批次</span>
                <el-button link type="primary" @click="loadBatches">刷新</el-button>
              </div>
            </template>
            <div class="filter-row">
              <el-select v-model="batchQuery.status" clearable placeholder="全部状态" @change="reloadBatches">
                <el-option label="已创建" value="CREATED"/>
                <el-option label="生成中" value="GENERATING"/>
                <el-option label="校验中" value="VALIDATING"/>
                <el-option label="待确认" value="PENDING_CONFIRMATION"/>
                <el-option label="部分发布" value="PARTIALLY_PUBLISHED"/>
                <el-option label="已发布" value="PUBLISHED"/>
                <el-option label="生成失败" value="GENERATION_FAILED"/>
                <el-option label="全部无效" value="INVALID"/>
                <el-option label="已取消" value="CANCELLED"/>
              </el-select>
              <CourseSelect v-if="!embedded" v-model="batchQuery.courseId" placeholder="筛选课程" @change="reloadBatches"/>
              <el-button @click="reloadBatches">查询</el-button>
            </div>
            <el-table v-loading="batchLoading" :data="batches" empty-text="暂无 AI 出题批次" row-key="id"
                      @row-click="openBatch">
              <el-table-column prop="id" label="批次" width="90"/>
              <el-table-column prop="scopeName" label="考查范围" min-width="150" show-overflow-tooltip/>
              <el-table-column prop="sourceType" label="来源" width="110">
                <template #default="{ row }">{{ sourceTypeText(row.sourceType) }}</template>
              </el-table-column>
              <el-table-column label="进度" width="120">
                <template #default="{ row }">{{ row.validCount || 0 }}/{{ row.totalCount || 0 }} 有效</template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="90">
                <template #default="{ row }">
                  <el-button link type="primary" @click.stop="openBatch(row)">审核</el-button>
                </template>
              </el-table-column>
            </el-table>
            <div class="pager">
              <el-pagination v-model:current-page="batchQuery.pageNo" v-model:page-size="batchQuery.pageSize"
                             layout="total, prev, pager, next" :total="batchTotal" @current-change="loadBatches"/>
            </div>
          </el-card>
        </div>
      </el-tab-pane>

      <el-tab-pane v-if="!embedded" label="视频智能处理" name="videos">
        <div class="workspace-grid">
          <div class="create-column">
            <el-card class="panel-card create-card" shadow="never">
              <template #header>
                <div class="card-title"><span>创建视频 AI 任务</span>
                  <el-tag type="success">P2-A/B</el-tag>
                </div>
              </template>
              <el-alert title="任务完成后可查看转写、分段摘要、知识点和复习重点，并从分析结果生成测验草稿。" type="success"
                        :closable="false" show-icon/>
              <el-form :model="videoForm" label-width="92px" class="workspace-form">
                <CourseCataloguePicker ref="videoPickerRef" v-model:course-id="videoForm.courseId"
                                       v-model:scope-id="videoForm.sectionId" v-model:media-id="videoForm.mediaId"
                                       :show-scope="false" :show-section="true" :show-target="false" show-media/>
                <el-form-item>
                  <el-button type="success" :loading="videoCreating" @click="createVideoTask">创建处理任务</el-button>
            ...
            <el-card class="panel-card create-card" shadow="never">
              <template #header>
                <div class="card-title"><span>创建章级综合测验草稿</span>
                  <el-tag type="warning">P2-C</el-tag>
                </div>
              </template>
              <el-alert title="章级测验聚合本章多个小节的视频分析结果，仍复用题目草稿审核、确认和发布流程。" type="warning"
                        :closable="false" show-icon/>
              <el-form :model="chapterQuizForm" label-width="92px" class="workspace-form">
                <CourseCataloguePicker ref="chapterQuizPickerRef" v-model:course-id="chapterQuizForm.courseId"
                                       v-model:scope-type="chapterQuizForm.scopeType"
                                       v-model:scope-id="chapterQuizForm.chapterId"
                                       v-model:target-biz-id="chapterQuizForm.targetBizId" :allow-section="false"
                                       :allow-chapter="true" :show-target="true" :show-media="false"
                                       @loaded="handleChapterCatalogueLoaded"/>
                <el-form-item label="视频任务" class="is-required">
                  <el-select v-model="chapterQuizForm.videoTaskIds" multiple collapse-tags collapse-tags-tooltip
                             :loading="chapterVideoTaskLoading" :disabled="!chapterQuizForm.chapterId"
                             placeholder="请选择本章已完成的视频任务" @change="handleChapterVideoTaskChange">
                    <el-option v-for="task in chapterVideoTasks" :key="task.id" :label="chapterVideoTaskLabel(task)"
                               :value="task.id"/>
                  </el-select>
                  <div class="form-tip">仅展示当前章节已完成的任务，最多选择 20 个视频。</div>
                </el-form-item>
                <el-form-item label="题型" class="is-required">
                  <el-checkbox-group v-model="chapterQuizForm.questionTypes">
                    <el-checkbox v-for="item in questionTypeOptions" :key="item.value" :label="item.value">{{
                        item.label
                      }}
                    </el-checkbox>
                ...
                <div class="form-row">
                  <el-form-item label="题目数量" class="is-required">
                    <el-input-number v-model="chapterQuizForm.questionCount" :min="1" :max="20"/>
                  </el-form-item>
                  <el-form-item label="难度" class="is-required">
                    <el-select v-model="chapterQuizForm.difficulty">
                      <el-option v-for="item in difficultyOptions" :key="item.value" :label="item.label"
                                 :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="每题分值" class="is-required">
                    <el-input-number v-model="chapterQuizForm.score" :min="1" :max="100"/>
                  </el-form-item>
                </div>
                <el-form-item>
                  <el-button type="warning" :loading="chapterQuizCreating" @click="createChapterQuiz">生成章级草稿
                  </el-button>
                  <el-button @click="resetChapterQuizForm">重置</el-button>
                </el-form-item>
              </el-form>
            </el-card>
          </div>
          <el-card class="panel-card" shadow="never">
            <template #header>
              <div class="card-title"><span>视频 AI 任务</span>
                <el-button link type="primary" @click="loadVideoTasks">刷新</el-button>
              </div>
            </template>
            <div class="filter-row">
              <el-select v-model="videoQuery.status" clearable placeholder="全部状态" @change="reloadVideoTasks">
                <el-option label="等待处理" value="CREATED"/>
                <el-option label="正在转写" value="TRANSCRIBING"/>
                <el-option label="转写完成" value="TRANSCRIBED"/>
                <el-option label="正在分析" value="ANALYZING"/>
                <el-option label="处理完成" value="COMPLETED"/>
                <el-option label="处理失败" value="FAILED"/>
              </el-select>
              <CourseSelect v-model="videoQuery.courseId" placeholder="筛选课程" @change="reloadVideoTasks"/>
              <el-button @click="reloadVideoTasks">查询</el-button>
            </div>
            <el-table v-loading="videoLoading" :data="videoTasks" empty-text="暂无视频 AI 任务" row-key="id"
                      @row-click="openVideoTask">
              <el-table-column prop="id" label="任务" width="90"/>
              <el-table-column prop="sectionName" label="视频小节" min-width="150" show-overflow-tooltip/>
              <el-table-column prop="mediaName" label="媒资" min-width="130" show-overflow-tooltip/>
              <el-table-column label="状态" width="110">
                <template #default="{ row }">
                  <el-tag :type="statusType(row.status, 'video')">{{ statusText(row.status, 'video') }}</el-tag>
                </template>
              </el-table-column>
              <el-table-column label="进度" width="90">
                <template #default="{ row }">{{ row.progress || 0 }}%</template>
              </el-table-column>
              <el-table-column label="重试" width="75">
              ...
              <el-table-column label="操作" width="110">
                <template #default="{ row }">
                  <el-button link type="primary" @click.stop="openVideoTask(row)">查看</el-button>
                  <el-button v-if="canRetryVideo(row)" link type="warning" :loading="videoRetryingId === String(row.id)"
                             @click.stop="retryVideoTaskRow(row)">重试
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
            <div class="pager">
              <el-pagination v-model:current-page="videoQuery.pageNo" v-model:page-size="videoQuery.pageSize"
                             layout="total, prev, pager, next" :total="videoTotal" @current-change="loadVideoTasks"/>
            </div>
          </el-card>
        </div>
      </el-tab-pane>

      <el-tab-pane v-if="!embedded" label="主观题 AI 评估" name="reviews">
        <el-card class="panel-card review-card" shadow="never">
          <template #header>
            <div class="card-title"><span>主观题人工复核工作台</span>
              <el-tag type="warning">P1-C</el-tag>
            </div>
          </template>
          <el-alert title="AI 仅提供建议分、得分点和置信度，正式成绩必须由课程创建者人工确认。" type="warning"
                    :closable="false" show-icon/>
          <div class="filter-row review-filter">
            <CourseSelect v-model="reviewQuery.courseId" placeholder="请选择审核课程（必填）"
                          @change="handleReviewCourseChange"/>
            <el-input v-model="reviewQuery.targetBizId" clearable placeholder="练习目录 ID"
                      @keyup.enter="reloadReviews"/>
            <el-input v-model="reviewQuery.studentId" clearable placeholder="学生 ID" @keyup.enter="reloadReviews"/>
            <el-select v-model="reviewQuery.manualReviewRecommended" clearable placeholder="是否建议复核">
              <el-option label="建议人工复核" :value="true"/>
              <el-option label="无需人工复核" :value="false"/>
            </el-select>
            <el-input-number v-model="reviewQuery.minConfidence" :min="0" :max="1" :step="0.1" :precision="1"
                             placeholder="最低置信度" controls-position="right"/>
            <el-input-number v-model="reviewQuery.maxConfidence" :min="0" :max="1" :step="0.1" :precision="1"
                             placeholder="最高置信度" controls-position="right"/>
            <el-button type="primary" @click="reloadReviews">查询</el-button>
          </div>
          <el-table v-loading="reviewLoading" :data="reviews"
                    :empty-text="reviewQuery.courseId ? '暂无待审核记录' : '请先选择课程并查询待审核记录'">
            <el-table-column prop="answerId" label="答案" width="80"/>
            <el-table-column prop="targetName" label="练习目录" min-width="130" show-overflow-tooltip/>
            <el-table-column prop="questionName" label="题干" min-width="220" show-overflow-tooltip/>
            <el-table-column prop="studentId" label="学生" width="85"/>
            <el-table-column label="AI 建议分" width="105">
              <template #default="{ row }">{{ row.aiSuggestedScore ?? '-' }}/{{ row.questionScore ?? '-' }}</template>
            </el-table-column>
            <el-table-column label="置信度" width="95">
              <template #default="{ row }">{{ confidenceText(row.confidence) }}</template>
            </el-table-column>
            <el-table-column label="复核建议" width="105">
              <template #default="{ row }">
                <el-tag :type="row.manualReviewRecommended ? 'warning' : 'info'">
                  {{ row.manualReviewRecommended ? '建议复核' : '可直接参考' }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column label="评估时间" width="155">
              <template #default="{ row }">{{ formatDateTime(row.evaluatedTime) }}</template>
            </el-table-column>
          ...
          <div class="pager">
            <el-pagination v-model:current-page="reviewQuery.pageNo" v-model:page-size="reviewQuery.pageSize"
                           layout="total, prev, pager, next" :total="reviewTotal" @current-change="loadReviews"/>
          </div>
        </el-card>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-drawer v-model="batchDrawerVisible" title="题目草稿批次详情" size="72%">
      <template v-if="currentBatch">
        <div class="detail-summary">
          <div><strong>{{ currentBatch.scopeName || `批次 ${currentBatch.id}` }}</strong>
            <p>来源：{{ sourceTypeText(currentBatch.sourceType) }}；有效题目 {{
                currentBatch.validCount || 0
              }}/{{ currentBatch.totalCount || 0 }}</p></div>
          <el-tag :type="statusType(currentBatch.status)">{{ statusText(currentBatch.status) }}</el-tag>
        </div>
        <el-alert v-if="currentBatch.failureReason" :title="currentBatch.failureReason" type="error" :closable="false"
                  show-icon/>
        <div class="drawer-actions">
          <el-button @click="refreshBatch">刷新</el-button>
          <el-button v-if="canConfirmBatch" type="primary" :loading="batchConfirming" @click="confirmBatch">
            确认全部有效草稿
          </el-button>
          <el-button v-if="canPublishBatch" type="success" :loading="batchPublishing" @click="publishBatch">
            发布已确认题目
          </el-button>
          <el-button v-if="canRetryBatch" type="warning" :loading="batchRetrying" @click="retryBatch">重试生成
          </el-button>
        </div>
        <el-card v-for="draft in currentBatch.drafts || []" :key="draft.id" class="draft-card" shadow="never">
          <div class="draft-head"><span>第 {{ draft.sequenceNo || '-' }} 题 · {{
              questionTypeText(draft.type)
            }} · {{ difficultyText(draft.difficulty) }}</span>
            <el-tag :type="statusType(draft.status, 'draft')">{{ statusText(draft.status, 'draft') }}</el-tag>
          </div>
          <div class="question-title">{{ draft.name || '暂无题干' }}</div>
          <div v-if="draft.options?.length" class="option-list">
            <div v-for="(option, index) in draft.options" :key="`${draft.id}-${index}`">
              {{ String.fromCharCode(65 + index) }}. {{ option }}
            </div>
          </div>
          <div class="answer-line">参考答案：{{ answerDisplayText(draft.answer, draft.type) }}<span>{{
              draft.score || 0
            }} 分</span></div>
          <div class="analysis">答案解析：{{ draft.analysis || '暂无解析' }}</div>
          <div v-if="draft.validationMessage" class="validation">校验提示：{{ draft.validationMessage }}</div>
          <div class="draft-actions">
            <el-button v-if="!['PUBLISHED', 'REJECTED'].includes(draft.status)" link type="primary"
                       @click="editDraft(draft)">编辑
            </el-button>
            <el-button v-if="draft.status === 'PENDING_CONFIRMATION'" link type="success"
                       :loading="draftActionId === String(draft.id)" @click="confirmDraft(draft)">确认
            </el-button>
            <el-button v-if="draft.status === 'PENDING_CONFIRMATION'" link type="danger"
                       :loading="draftActionId === String(draft.id)" @click="rejectDraft(draft)">驳回
            </el-button>
            <el-button v-if="draft.status !== 'PUBLISHED'" link type="danger"
                       :loading="draftActionId === String(draft.id)" @click="deleteDraft(draft)">删除
            </el-button>
          </div>
        </el-card>
      </template>
    ...
    <el-drawer v-model="videoDrawerVisible" title="视频 AI 任务详情" size="72%">
      <template v-if="currentVideo">
        <div class="detail-summary">
          <div><strong>{{ currentVideo.sectionName || `任务 ${currentVideo.id}` }}</strong>
            <p>媒资：{{ currentVideo.mediaName || '-' }}；进度：{{ currentVideo.progress || 0 }}%</p></div>
          <el-tag :type="statusType(currentVideo.status, 'video')">{{
              statusText(currentVideo.status, 'video')
            }}
          </el-tag>
        </div>
        <el-alert v-if="currentVideo.failureReason" :title="currentVideo.failureReason" type="error" :closable="false"
                  show-icon/>
        <div class="drawer-actions">
          <el-button @click="refreshVideo">刷新</el-button>
          <el-button v-if="canRetryVideo(currentVideo)" type="warning"
                     :loading="videoRetryingId === String(currentVideo.id)" @click="retryVideo">重试处理
          </el-button>
          <el-button v-if="currentVideo.status === 'COMPLETED'" type="primary" @click="openVideoQuiz">生成测验草稿
          </el-button>
        </div>
        <el-tabs class="result-tabs">
          <el-tab-pane label="内容摘要" name="summary"><h3>视频简介</h3>
            <p>{{ currentVideo.videoIntroduction || '暂无' }}</p>
            <h3>核心内容</h3>
            <p>{{ currentVideo.coreContent || '暂无' }}</p>
            <h3>关键结论</h3>
            <p>{{ (currentVideo.keyConclusions || []).join('；') || '暂无' }}</p>
            <h3>适合人群</h3>
            <p>{{ (currentVideo.suitableLearners || []).join('；') || '暂无' }}</p>
            <h3>复习重点</h3>
            <p>{{ (currentVideo.reviewPoints || []).join('；') || '暂无' }}</p></el-tab-pane>
          <el-tab-pane label="分段摘要" name="sections">
            <div v-for="(item, index) in currentVideo.sectionSummaries || []" :key="`${item.title}-${index}`"
                 class="knowledge-item">
              <div class="knowledge-title">
                <el-tag size="small">{{ formatDuration(item.startMs) }} - {{ formatDuration(item.endMs) }}</el-tag>
                <strong>{{ item.title || `第 ${index + 1} 段` }}</strong></div>
              <p>{{ item.summary || '暂无摘要' }}</p></div>
            <el-empty v-if="!currentVideo.sectionSummaries?.length" description="暂无分段摘要"/>
          </el-tab-pane>
          <el-tab-pane label="知识点" name="knowledge">
            <div v-for="(item, index) in currentVideo.knowledgePoints || []" :key="`${item.name}-${index}`"
                 class="knowledge-item">
              <div class="knowledge-title"><strong>{{
                  item.name || `知识点 ${index + 1}`
                }}</strong><span>重要度 {{
                  importanceText(item.importance)
                }} · 难度 {{ knowledgeDifficultyText(item.difficulty) }}</span></div>
              <p>{{ item.description || '暂无说明' }}</p>
              <div v-if="item.prerequisites?.length">前置知识：{{ item.prerequisites.join('、') }}</div>
            </div>
            <el-empty v-if="!currentVideo.knowledgePoints?.length" description="暂无知识点"/>
          </el-tab-pane>
          <el-tab-pane label="转写全文" name="transcript">
            <div class="transcript-box">
              <div v-if="currentVideo.fullText">{{ currentVideo.fullText }}</div>
              <el-empty v-else description="暂无转写文本"/>
            </div>
            <div v-if="currentVideo.transcriptSegments?.length" class="transcript-box">
              <div v-for="(item, index) in currentVideo.transcriptSegments" :key="index" class="transcript-segment">
                <span>{{ formatDuration(item.startMs) }}</span>
                <p>{{ item.text }}</p></div>
            </div>
          </el-tab-pane>
        </el-tabs>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑题目草稿" width="620px">
      <el-form :model="editingDraft" label-width="82px">
        <el-form-item label="题干" class="is-required">
          <el-input v-model="editingDraft.name" type="textarea" rows="3" maxlength="1000" show-word-limit/>
        </el-form-item>
        <el-form-item label="题型">
          <el-select v-model="editingDraft.type">
            <el-option v-for="item in questionTypeOptions" :key="item.value" :label="item.label" :value="item.value"/>
          </el-select>
        </el-form-item>
        <el-form-item label="选项" v-if="editingDraft.type !== 4 && editingDraft.type !== 5">
          <el-input v-model="editingOptionsText" type="textarea" rows="3" placeholder="每行一个选项"/>
        </el-form-item>
        <el-form-item label="答案" class="is-required">
          <el-input v-model="editingDraft.answer" maxlength="10000" show-word-limit/>
        </el-form-item>
        <div class="form-row">
          <el-form-item label="难度">
            <el-select v-model="editingDraft.difficulty">
              <el-option v-for="item in difficultyOptions" :key="item.value" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="分值">
            <el-input-number v-model="editingDraft.score" :min="1" :max="100"/>
          </el-form-item>
        </div>
        <el-form-item label="解析" class="is-required">
          <el-input v-model="editingDraft.analysis" type="textarea" rows="4" maxlength="300" show-word-limit/>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="editDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="editSaving" @click="saveDraftEdit">保存并重新校验</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="videoQuizDialogVisible" title="根据视频生成测验草稿" width="560px">
      <el-form :model="videoQuizForm" label-width="100px">
        <CourseCataloguePicker ref="videoQuizPickerRef" :course-id="currentVideo?.courseId"
                               :scope-id="currentVideo?.sectionId" v-model:target-biz-id="videoQuizForm.targetBizId"
                               course-readonly :show-scope="false" :show-section="false" :show-media="false"/>
        <el-form-item label="题型">
          <el-checkbox-group v-model="videoQuizForm.questionTypes">
            <el-checkbox v-for="item in questionTypeOptions" :key="item.value" :label="item.value">{{
                item.label
              }}
            </el-checkbox>
        ...
        <div class="form-row">
          <el-form-item label="题目数量">
            <el-input-number v-model="videoQuizForm.questionCount" :min="1" :max="20"/>
          </el-form-item>
          <el-form-item label="难度">
            <el-select v-model="videoQuizForm.difficulty">
              <el-option v-for="item in difficultyOptions" :key="item.value" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="每题分值">
            <el-input-number v-model="videoQuizForm.score" :min="1" :max="100"/>
          </el-form-item>
        </div>
      </el-form>
      <template #footer>
        <el-button @click="videoQuizDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="quizCreating" @click="createVideoQuiz">生成草稿</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="reviewDialogVisible" title="确认主观题 AI 评估" width="680px">
      <template v-if="currentReview">
        <div class="review-question"><h3>{{ currentReview.questionName }}</h3>
          <p><strong>学生答案：</strong>{{ currentReview.studentAnswer || '未作答' }}</p>
          <p><strong>参考答案：</strong>{{ currentReview.standardAnswer || '-' }}</p>
          <div class="review-points">
            <div><b>命中得分点</b><span v-for="item in currentReview.matchedPoints || []" :key="item"
                                        class="point good">{{ item }}</span></div>
            <div><b>缺失得分点</b><span v-for="item in currentReview.missingPoints || []" :key="item"
                                        class="point warn">{{ item }}</span></div>
            <div><b>问题表述</b><span v-for="item in currentReview.incorrectStatements || []" :key="item"
                                      class="point bad">{{ item }}</span></div>
          </div>
          <p class="suggestion">改进建议：{{ currentReview.improvementSuggestion || '暂无' }}</p>
          <el-form label-width="100px">
            <el-form-item label="人工确认得分">
              <el-input-number v-model="reviewScore" :min="0" :max="currentReview.questionScore || 100"/>
            </el-form-item>
          </el-form>
        </div>
      </template>
      <template #footer>
        ...
        <el-button type="primary" :loading="reviewSaving" @click="confirmReview">确认最终得分</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {
  ...
  queryAiQuestionBatches,
  getAiQuestionBatch,
  confirmAiQuestionBatch,
  publishAiQuestionBatch,
  retryAiQuestionBatch,
  updateAiQuestionDraft,
  confirmAiQuestionDraft,
  rejectAiQuestionDraft,
  deleteAiQuestionDraft,
  createVideoAiTask,
  queryVideoAiTasks,
  getVideoAiTask,
  retryVideoAiTask,
  createVideoQuizDrafts,
  createChapterQuizDrafts,
  queryPendingAiReviews,
  confirmPracticeAiReview,
  unwrapAiResponse,
  getAiErrorMessage
...
const props = defineProps({
  /** 嵌入课程编辑页时复用工作台能力，独立工作台保持原有页面结构和流程。 */
  embedded: {type: Boolean, default: false},
  initialCourseId: [String, Number],
  initialScopeType: {type: String, default: 'SECTION'},
  initialScopeId: [String, Number],
  initialTargetBizId: [String, Number],
  initialTab: {type: String, default: 'drafts'}
})
const emit = defineEmits(['published'])
const embedded = props.embedded
const activeTab = ref(props.initialTab || 'drafts')
const initialCourseId = props.initialCourseId ? String(props.initialCourseId) : ''
const initialScopeId = props.initialScopeId ? String(props.initialScopeId) : ''
const initialTargetBizId = props.initialTargetBizId ? String(props.initialTargetBizId) : ''
const initialScopeType = props.initialScopeType || 'SECTION'
const batchForm = reactive({
  courseId: initialCourseId,
  scopeType: initialScopeType,
  scopeId: initialScopeId,
  targetBizId: initialTargetBizId,
...
const chapterQuizForm = reactive({
  ...
  difficulty: 2,
  score: 5,
  requestId: createClientRequestId('chapter-quiz')
})
const videoQuizForm = reactive({
  ...
  difficulty: 2,
  score: 5,
  requestId: createClientRequestId('video-quiz')
})
const batchQuery = reactive({pageNo: 1, pageSize: 10, status: '', courseId: initialCourseId})
...
const batchPickerRef = ref(null), videoPickerRef = ref(null), videoQuizPickerRef = ref(null),
    chapterQuizPickerRef = ref(null)
const batchDrawerVisible = ref(false), videoDrawerVisible = ref(false), editDialogVisible = ref(false),
    videoQuizDialogVisible = ref(false), reviewDialogVisible = ref(false)
const editingDraft = reactive({}), editingOptionsText = ref(''), editSaving = ref(false), quizCreating = ref(false),
    chapterQuizCreating = ref(false), reviewSaving = ref(false), reviewScore = ref(0)
const batchConfirming = ref(false), batchPublishing = ref(false), batchRetrying = ref(false), draftActionId = ref(''),
    videoRetryingId = ref('')
...
/** 统一判断接口操作是否成功并展示结果。 */
const ensureSuccess = (response, successMessage) => {
  if (response?.code !== 200) {
    ElMessage.error(getAiErrorMessage(response, '操作失败，请稍后重试'));
    return false
  }
...
const resetBatchForm = async () => {
  Object.assign(batchForm, {
    courseId: embedded ? initialCourseId : '',
    scopeType: embedded ? initialScopeType : 'SECTION',
    scopeId: embedded ? initialScopeId : '',
    targetBizId: embedded ? initialTargetBizId : '',
    sourceType: 'MANUAL_TOPIC',
    knowledgePoints: '',
    materialText: '',
    questionTypes: [1, 4],
  ...
  if (embedded) {
    await nextTick()
    await batchPickerRef.value?.loadCatalogues()
  }
}
...
const publishBatch = async () => {
  ...
  try {
    if (ensureSuccess(await publishAiQuestionBatch(batchId), '题目已发布')) {
      ...
      await loadBatches()
      if (embedded) emit('published', batchId)
    }
  } finally {
...
const loadChapterVideoTasks = async () => {
  ...
  try {
    const response = await queryVideoAiTasks({
      pageNo: 1,
      pageSize: 100,
...
const openReview = row => {
  ...
  reviewScore.value = row.aiSuggestedScore ?? 0;
...
.embedded-workspace :deep(.ai-tabs) {
  margin-top: 0;
}
...
.ai-hero {
  ...
  border-radius: 16px;
  color: #fff;
  background: linear-gradient(135deg, #1f4d7a, #6c5ce7);
  box-shadow: 0 14px 35px rgba(64, 80, 170, .18);
}
...
.workspace-grid {
  display: grid;
  grid-template-columns: minmax(360px, .92fr) minmax(560px, 1.55fr);
  gap: 20px;
}
...
.workspace-form :deep(.el-select), .workspace-form :deep(.el-input-number) {
  width: 100%;
}
