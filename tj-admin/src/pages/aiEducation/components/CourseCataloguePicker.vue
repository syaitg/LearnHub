<template>
  <div class="catalogue-picker">
    ...
    <el-alert v-if="showTarget && loaded && chapters.length && !targetPracticeCount"
              title="当前出题范围没有可用的练习/测试发布目录，请先在课程目录中创建练习或测试目录。" type="warning"
              :closable="false" show-icon/>
    <template v-if="chapters.length">
      <el-form-item v-if="showScope" label="出题范围" class="is-required">
        <el-radio-group v-model="scopeTypeValue" :disabled="scopeReadonly" @change="handleScopeTypeChange">
      ...
      <el-form-item v-if="showScope && scopeTypeValue === 'CHAPTER'" label="章节" class="is-required">
        <el-select v-model="scopeIdValue" filterable :disabled="scopeReadonly" placeholder="请选择章节" @change="handleChapterChange">
          <el-option v-for="chapter in chapters" :key="chapter.id" :label="chapter.name" :value="chapter.id"/>
        </el-select>
      </el-form-item>
      <el-form-item v-if="(showScope || showSection) && scopeTypeValue === 'SECTION'" label="视频小节"
                    class="is-required">
        <el-select v-model="scopeIdValue" filterable :disabled="scopeReadonly" placeholder="请选择小节" @change="handleSectionChange">
          <el-option-group v-for="chapter in chapters" :key="chapter.id" :label="chapter.name">
            <el-option v-for="section in videoSections(chapter)" :key="section.id" :label="section.name"
                       :value="section.id"/>
          </el-option-group>
        </el-select>
      </el-form-item>
      <el-form-item v-if="showMedia" label="媒资" class="is-required">
        <el-select v-model="mediaIdValue" filterable placeholder="请先选择包含视频的小节" @change="handleMediaChange">
          <el-option v-for="section in selectableMediaSections" :key="section.mediaId"
                     :label="`${section.name}（媒资 ${section.mediaId}）`" :value="section.mediaId"/>
        </el-select>
      </el-form-item>
      <el-form-item v-if="showTarget" label="发布目录" class="is-required">
        <el-select
            ...
            filterable
            :disabled="targetReadonly || !targetPracticeCount"
        ...
        >
          <el-option-group v-for="chapter in selectableTargetChapters" :key="chapter.id" :label="chapter.name">
            <el-option v-for="practice in practiceSections(chapter)" :key="practice.id" :label="practice.name"
                       :value="practice.id"/>
          </el-option-group>
        </el-select>
      </el-form-item>
...
const props = defineProps({
  ...
  scopeReadonly: {type: Boolean, default: false},
  targetReadonly: {type: Boolean, default: false},
...
const reset = () => {
  ...
  const preservedScopeId = props.scopeReadonly && props.scopeId ? String(props.scopeId) : ''
  const preservedTargetBizId = props.targetReadonly && props.targetBizId ? String(props.targetBizId) : ''
  manualMode.value = false
  clearCatalogueSelection()
  scopeTypeValue.value = props.allowChapter && !props.allowSection
      ? 'CHAPTER'
      : (props.allowChapter ? (props.scopeType || 'SECTION') : 'SECTION')
  if (preservedScopeId || (!props.showScope && !props.showSection && props.scopeId)) {
    scopeIdValue.value = preservedScopeId || String(props.scopeId)
  }
  if (preservedTargetBizId) targetBizIdValue.value = preservedTargetBizId
  emitValues()
}
...
watch(() => props.scopeId, value => {
  scopeIdValue.value = value || ''
})
watch(() => props.targetBizId, value => {
  targetBizIdValue.value = value || ''
})
watch(() => props.mediaId, value => {
  mediaIdValue.value = value || ''
})
...
.catalogue-picker :deep(.el-select) {
  width: 100%;
}

.catalogue-picker .el-alert {
  margin-bottom: 14px;
}
</style>
