...
public class MongoDBChatMemoryRepository implements ChatMemoryRepository, MyChatMemoryRepository {
    ...
    public void optimization(String conversationId) {
        ...
        Query query = Query.query(Criteria.where("conversationId").is(conversationId));
        ChatRecord record = this.mongoTemplate.findOne(query, ChatRecord.class);
        if (record == null || record.getMessages() == null || record.getMessages().isEmpty()) {
            return;
        }
        List<String> messages = record.getMessages();
        int retainedSize = Math.max(0, messages.size() - 2);
        record.setMessages(List.copyOf(messages.subList(0, retainedSize)));
