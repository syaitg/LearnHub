...
public class ChatSessionServiceImpl extends ServiceImpl<ChatSessionMapper, ChatSession> implements ChatSessionService {
    ...
    public SessionVO createSession(Integer num) {
        ...
        // 随机生成3个热门问题
        sessionVO.setExamples(this.randomExamples(num));
    ...
    public List<SessionVO.Example> hotExamples(Integer num) {
        return this.randomExamples(num);
    }

    private List<SessionVO.Example> randomExamples(Integer num) {
        List<SessionVO.Example> examples = this.sessionProperties.getExamples();
        if (CollUtil.isEmpty(examples) || num == null || num <= 0) {
            return List.of();
        }
        return RandomUtil.randomEleList(examples, Math.min(num, examples.size()));
    }
