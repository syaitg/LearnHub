...
public class PrePlaceOrder {
    ...
    @JsonPropertyDescription("是否为免费课程")
    ...
    public static PrePlaceOrder of(OrderConfirmVO orderConfirmVO) {
        return of(orderConfirmVO, false);
    }

    public static PrePlaceOrder of(OrderConfirmVO orderConfirmVO, boolean freeCourse) {
        // 交易服务降级或异常响应不能被包装成金额为0的“成功预下单”。
        if (orderConfirmVO == null
                || orderConfirmVO.getOrderId() == null
                || CollUtil.isEmpty(orderConfirmVO.getCourses())) {
            return null;
        }
        // 订单总金额
