package com.tianji.aigc.controller;

import com.tianji.aigc.dto.ChatDTO;
import com.tianji.aigc.service.ChatService;
import com.tianji.aigc.vo.ChatEventVO;
import com.tianji.aigc.vo.TemplateVO;
import com.tianji.common.annotations.NoWrapper;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.http.MediaType;
import org.springframework.web.bind.annotation.*;
import reactor.core.publisher.Flux;

@Slf4j
@RestController
@RequestMapping("/chat")
@RequiredArgsConstructor
public class ChatController {

    private final ChatService chatService;

    // 优化：将文本的模版写入到nacos中
    private static final TemplateVO TEMPLATE_VO = new TemplateVO();

    @NoWrapper // 这个注解是本项目自定义的注解，用于表明响应的结果不需要包装，直接返回（不需要外层的code、msg、data那一层结构，直接返回 data: ... ）
               // 因为是流式的，所以不需要包装外面那一层。
    @PostMapping(produces = MediaType.TEXT_EVENT_STREAM_VALUE) // 响应类型为：text/event-stream 事件型(流式输出)
    public Flux<ChatEventVO> chat(@RequestBody ChatDTO chatDTO) { // Flux表示流式输出
        return this.chatService.chat(chatDTO.getQuestion(), chatDTO.getSessionId());
    }

    @PostMapping("/stop")
    public void stop(@RequestParam("sessionId") String sessionId) {
        this.chatService.stop(sessionId);
    }

    @PostMapping("/text")
    public String chatText(@RequestBody String question) {
        return this.chatService.chatText(question);
    }

    @GetMapping("/templates")
    public TemplateVO getTemplates() {
        return TEMPLATE_VO;
    }

}
