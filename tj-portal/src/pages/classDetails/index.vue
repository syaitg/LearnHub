  ...
  <div class="classDetailsWrapper">
    ...
    <div class="DetailsContent container fx">
      <div class="leftCont bg-wt">
        ...
        <!-- 问答模块 -->
        <ClassAsk v-if="hasToken() && isSignUp" v-show="actId == 3" :id="detailsId" :title="baseDetailsData.name"></ClassAsk>
        <!-- 笔记模块 -->
        <Note  v-if="hasToken() && isSignUp" v-show=" actId == 4" :id="detailsId"></Note>
...
import { dataCacheStore, hasToken } from "@/store"
...
const tableBar = computed(() => {
  return hasToken() && isSignUp.value ? logData : noLogData
})
    ...
    .then((res) => {
      if (res.code == 200) {
        ...
        for (let i = 0; i < data.length; i += 2) {
          catchArr.push(data.slice(i, i + 2))
        }
...
// 马上学习
const goLearning = () => {
  router.push({path: '/learning/index', query: {id: detailsId.value}})
}
