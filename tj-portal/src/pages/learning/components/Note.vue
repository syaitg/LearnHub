...
<script setup>
import { onMounted, ref, reactive, watch, watchEffect, inject } from 'vue'
import { getAllNotes, addNotes, delNote, updateNotes, notesGathers, unNotesGathers } from "@/api/notes.js"
...
// 播放小节切换后同步笔记表单和列表查询的课程上下文。
const syncNoteContext = () => {
  noteParams.sectionId = currentPlayData?.sectionId ?? null
  noteParams.courseId = currentPlayData?.courseId ?? null
  noteParams.chapterId = currentPlayData?.chapterId ?? null
  params.value.sectionId = currentPlayData?.sectionId ?? null
  params.value.courseId = currentPlayData?.courseId ?? null
}
