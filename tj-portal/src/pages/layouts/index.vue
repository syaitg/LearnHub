  ...
  <div class="layoutsWrapper fx-fd-col" @click.stop="handleHidePopover">
    ...
    <div class="aiSupernatant">
      ...
      <div v-if="!isShow && !dialogVisible" class="superCon">
        ...
        >
          <template #content>
            <div v-if="token">哈！LearnHub 不可泄露<br />除非你来问我</div>
            <div v-else>哇！我是LearnHub AI助理<br />你没有登录可不能和我聊哦～</div>
          </template>
          <div>
      ...
      <template #header>
        <div class="dialogHeader" @mousedown="startDrag">
          <div :id="titleId" class="title">
            <span class="font">LearnHub AI助理</span>
            <button
              :class="
      ...
      <div class="chitchatBox">
        ...
        >
          <div class="historyCon" @click.stop>
            <div v-if="historyData.length === 0" class="emptyHistory">
              暂无历史记录
            </div>
            ...
            <div v-if="historyData.length > 0" class="totleNum">
              仅展示最近30条对话
            </div>
          </div>
        </el-popover>
                          ...
                          <div class="cardList">
                            ...
                            <div class="cardBtn">
                              <span
                                ...
                                >{{ item.orderText.freeCourse ? '立即报名' : '立即下单' }}</span
                              >
                            </div>
                          </div>
                        </div>
...
import { enrolledFreeCourse, setOrder } from '@/api/order.js';
...
const renderMarkdown = (content) => {
  ...
  try {
    ...
    // A malformed legacy record must not interrupt the rendering of the whole chat window.
    return md.utils.escapeHtml(normalizedContent);
  }
};
...
const getList = async () => {
  ...
  if (isActive.value !== '') {
    historyData.value.forEach((item, index) => {
      item.list.forEach((session, i) => {
        if (session.sessionId === baseData.value.sessionId) {
...
const getCourseIds = (params) => {
  if (!params || typeof params !== 'object' || Array.isArray(params)) {
    return [];
  }

  const ids = [];
  Object.entries(params).forEach(([key, course]) => {
    if (
      course &&
    ...
    const legacyId = /^courseInfo_(\d+)$/.exec(key)?.[1];
...
const getCourseDetails = async (ids) => {
  ...
  const details = await Promise.all(validIds.map((id) => getDetails(id)));
...
const normalizeOrder = (order) => {
  ...
  return {
    ...
    payAmount: toAmount(order.payAmount),
  };
};
const normalizeHistoryMessage = (message) => {
  ...
  if (message.type !== 'ASSISTANT') return normalizedMessage;

  const messageParams =
    message.params && typeof message.params === 'object' && !Array.isArray(message.params)
      ? message.params
      : {};
  const prePlaceOrder = normalizeOrder(messageParams.prePlaceOrder);
  if (prePlaceOrder && Array.isArray(prePlaceOrder.courseIds)) {
    const ids = prePlaceOrder.courseIds.filter(
      (courseId) => courseId !== null && courseId !== undefined && courseId !== ''
    );
    return {
      ...normalizedMessage,
      ids,
      orderText: prePlaceOrder,
      cardType: 1,
      cardOrderDetails: [],
    };
  }
  ...
  return {
    ...normalizedMessage,
    ids: getCourseIds(messageParams),
    cardDetails: [],
  };
};
const hydrateHistoryCourseCards = async (messages, loadVersion) => {
  await Promise.allSettled(
    messages.map(async (message, index) => {
      if (!message || message.type !== 'ASSISTANT' || message.ids.length === 0) return;

      const details = await getCourseDetails(message.ids);
      if (loadVersion !== historyLoadVersion || !chatDataArr.value[index]) return;

      const detailField = message.cardType === 1 ? 'cardOrderDetails' : 'cardDetails';
      chatDataArr.value[index] = {
        ...chatDataArr.value[index],
        [detailField]: details,
      };
    })
...
const getListData = async (id) => {
  ...
  const res = await getChatHistory({ sessionId: id });
  if (!res || res.code !== 200 || !Array.isArray(res.data)) {
    listDataArr.value = [];
    throw new Error('Invalid history session response');
  }

  listDataArr.value = res.data;
  const processedData = listDataArr.value
    .map((message) => normalizeHistoryMessage(message))
    .filter(Boolean);
  if (currentLoadVersion !== historyLoadVersion) return;

  chatDataArr.value = processedData;
  await nextTick();
  void hydrateHistoryCourseCards(processedData, currentLoadVersion);
};
    ...
    eventSource.value = fetchEventSource(`/api/ais/chat`, {
      ...
      onmessage(msg) {
        ...
        if (msgData.eventType === 1003) {
          ...
          } else if (prePlaceOrder.value && prePlaceOrder.value.courseIds) {
            planDataIds.value = [];
            assistantObj = {
              ...assistantObj,
...
const handleOrder = async (val) => {
  // 免费课程不能创建支付订单，需要直接调用免费报名接口
  if (val.freeCourse) {
    if (!Array.isArray(val.courseIds) || val.courseIds.length !== 1) {
      ElMessage({
        message: '免费课程需要逐门报名',
        type: 'error',
      });
    ...
    const freeResult = await enrolledFreeCourse(val.courseIds[0]);
    if (freeResult.code === 200) {
      ElMessage({
        message: '报名成功',
        type: 'success',
      });
...
const handleHistory = () => {
  ...
  visible.value = true;
  clearData();
  nextTick(() => resTextHeight());

  void getList().catch(() => {
    ElMessage.error('历史记录刷新失败，已保留当前列表');
  });
};
...
// 触发历史记录
const handleActive = async (sessionId, index, i) => {
  // History navigation must remain available even when the previous stream got stuck.
  cancelActiveGeneration();
  if (sessionId === null || sessionId === undefined || sessionId === '') {
    ElMessage.error('该历史会话缺少有效的会话编号');
    return false;
  }
