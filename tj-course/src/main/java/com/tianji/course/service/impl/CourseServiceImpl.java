...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements ICourseService {
    ...
    public CourseAndSectionVO queryCourseAndCatalogById(Long courseId) {
        ...
        // 4.查询教师信息
        List<UserDTO> teachers = CollUtils.isEmpty(course.getTeacherIds())
                ? Collections.emptyList()
                : userClient.queryUserByIds(course.getTeacherIds());
        ...
        if (CollUtils.isNotEmpty(catas)) {
            for (CatalogueDTO c : catas) {
                ...
                List<SectionVO> sections = CollUtils.isEmpty(c.getSections())
                        ? new ArrayList<>()
                        : BeanUtils.copyToList(c.getSections(), SectionVO.class);
        ...
        Map<Long, LearningRecordDTO> rMap = records.stream()
                ...
                .filter(record -> record.getSectionId() != null)
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery, Long creater) {
        ...
        queryWrapper.eq(creater != null, Course::getCreater, creater);
        //1.1.课程条件-更新时间
