...
public class CourseDraftServiceImpl extends ServiceImpl<CourseDraftMapper, CourseDraft> implements ICourseDraftService {
    ...
    public CourseBaseInfoVO getCourseBaseInfo(Long id, Boolean see) {
        ...
        if (see) {
            ...
            if (course != null) {
                ...
                //1.5.设置报名人数
                courseBaseInfoVO.setEnrollNum(coursePurchaseInfoDTO == null ? 0 : NumberUtils.null2Zero(coursePurchaseInfoDTO.getEnrollNum()));
                //1.6.设置学习人数
                courseBaseInfoVO.setStudyNum(NumberUtils.null2Zero(learningClient.countLearningLessonByCourse(id)));
                //1.7.设置退款人数
                courseBaseInfoVO.setRefundNum(coursePurchaseInfoDTO == null ? 0 : NumberUtils.null2Zero(coursePurchaseInfoDTO.getRefundNum()));
                //1.8.设置实付金额
                courseBaseInfoVO.setRealPayAmount(coursePurchaseInfoDTO == null ? 0 : NumberUtils.null2Zero(coursePurchaseInfoDTO.getRealPayAmount()));
                //1.9.设置课程详情
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery) {
        return queryForPage(coursePageQuery, null);
    }
     ...
     * @param creater 课程创建者 ID，为空时保持原有全量查询语义
     * @return 课程分页数据
     */
    @Override
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery, Long creater) {
        //1.课程草稿分页查询条件
        LambdaQueryWrapper<CourseDraft> queryWrapper =
                SqlWrapperUtils.toLambdaQueryWrapper(coursePageQuery, CourseDraft.class);
        queryWrapper.eq(creater != null, CourseDraft::getCreater, creater);
        //1.1课程查询条件-更新时间
