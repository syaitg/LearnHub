...
public class CourseCatalogueDraftServiceImpl extends ServiceImpl<CourseCatalogueDraftMapper, CourseCatalogueDraft> implements ICourseCatalogueDraftService {
    ...
    public void save(Long courseId, List<CataSaveDTO> cataSaveDTOS, Integer step) {
        if (CollUtils.isEmpty(cataSaveDTOS)) {
    ...
    @Override
    public CatalogueDetailDTO queryCatalogueDetail(Long id) {
        CourseCatalogueDraft catalogue = getById(id);
        return catalogue == null ? null : BeanUtils.toBean(catalogue, CatalogueDetailDTO.class);
    }

    @Override
    public List<CatalogueDetailDTO> queryCatalogueDetails(List<Long> ids) {
        if (CollUtils.isEmpty(ids)) {
            return CollUtils.emptyList();
        }
        List<CourseCatalogueDraft> catalogues = lambdaQuery()
                .in(CourseCatalogueDraft::getId, ids)
                .list();
        return BeanUtils.copyList(catalogues, CatalogueDetailDTO.class);
    }
    ...
    public void copySubjectToShelf(Long courseId, Boolean isFirstShelf) {
        ...
        //2.重新上架前清理正式课程中已有的题目关系，避免清空题目或删除目录后旧关系残留。
        LambdaQueryWrapper<CourseCatalogue> shelfCataWrapper = Wrappers.lambdaQuery(CourseCatalogue.class)
                .eq(CourseCatalogue::getCourseId, courseId)
                .in(CourseCatalogue::getType, Arrays.asList(
                        CourseConstants.CataType.SECTION, CourseConstants.CataType.PRATICE));
        List<CourseCatalogue> shelfCatalogues = courseCatalogueMapper.selectList(shelfCataWrapper);
        Set<Long> bizIds = new LinkedHashSet<>();
        shelfCatalogues.forEach(catalogue -> bizIds.add(catalogue.getId()));
        courseCataSubjectDrafts.forEach(subject -> bizIds.add(subject.getCataId()));
        if (!CollUtils.isEmpty(bizIds)) {
            examClient.deleteQuestionBizInfoByBizIds(new ArrayList<>(bizIds));
        }

        //3.将新的题目关系上架；空题目列表代表该目录暂未配置题目。
        if (!CollUtils.isEmpty(subjects)) {
    ...
    private List<CataVO> queryCourseCatalogues(Long courseId, Boolean withPractice) {
        LambdaQueryWrapper<CourseCatalogueDraft> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(CourseCatalogueDraft::getCourseId, courseId);
        if (Boolean.FALSE.equals(withPractice)) {
        ...
        CourseDraft courseDraft = courseDraftService.getById(courseId);
        boolean noUpShelf = courseDraft == null
        ...
        Map<Long, CourseCatalogueDraft> chapterIdAndMaxSectionMap =
                noUpShelf
                        ...
                        courseCatalogueDrafts.parallelStream()
                                .filter(ccd -> ccd.getType() == CourseConstants.CataType.SECTION)
                                .filter(ccd -> !Boolean.TRUE.equals(ccd.getCanUpdate()))
                                .filter(ccd -> ccd.getParentCatalogueId() != null && ccd.getCIndex() != null)
                                .collect(Collectors.groupingBy(CourseCatalogueDraft::getParentCatalogueId,
                                        Collectors.collectingAndThen(
                                                ...
                                                Optional::get)));
        int maxChapterIndex = noUpShelf
                ? 0
                : courseCatalogueDrafts.stream()
                .filter(ccd -> ccd.getType() == CourseConstants.CataType.CHAPTER)
                .filter(ccd -> !Boolean.TRUE.equals(ccd.getCanUpdate()))
