...
import com.tianji.user.domain.dto.CurrentUserPasswordDTO;
import com.tianji.user.domain.dto.CurrentUserProfileDTO;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public UserDetailVO myInfo() {
        ...
        vo.setType(type == null ? null : type.getValue());
        // 3.2.详情信息
        if (type == null) {
            return vo;
        }
    ...
    @Override
    public void updateCurrentUserProfile(CurrentUserProfileDTO profileDTO) {
        Long userId = UserContext.getUser();
        if (userId == null) {
            throw new UnauthorizedException(USER_ID_NOT_EXISTS);
        }
        AssertUtils.isNotNull(detailService.getById(userId), USER_ID_NOT_EXISTS);
        UserDetail detail = new UserDetail();
        detail.setId(userId);
        detail.setName(profileDTO.getName().trim());
        detail.setGender(profileDTO.getGender());
        detail.setIcon(profileDTO.getIcon());
        detail.setJob(profileDTO.getJob());
        detail.setIntro(profileDTO.getIntro());
        detail.setPhoto(profileDTO.getPhoto());
        detailService.updateById(detail);
    }

    @Override
    public void updateCurrentUserPassword(CurrentUserPasswordDTO passwordDTO) {
        Long userId = UserContext.getUser();
        if (userId == null) {
            throw new UnauthorizedException(USER_ID_NOT_EXISTS);
        }
        ...
        updatedUser.setPassword(passwordEncoder.encode(passwordDTO.getPassword()));
        AssertUtils.isTrue(updateById(updatedUser), USER_ID_NOT_EXISTS);
    }
